...
работающую с эффектами как traverse_, , но параллельно с накоплением эффектов «восстанавливающую» сворачиваемую структуру в виде списка:
...
traverse2list :: (Foldable t, Applicative f) => (a -> f b) -> t a -> f [b]
traverse2list f a = foldr (\x bs -> pure (:) <*> f x <*> bs) (pure []) a